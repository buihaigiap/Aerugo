...
const MemberListItem: React.FC<MemberListItemProps> = ({
  ...
  const handleRoleChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const capitalizedRole = (newRoleValue.charAt(0).toUpperCase() +
      newRoleValue.slice(1)) as OrganizationRole;
  ...
  const canManage =
    ...
    (currentUserRole === "admin" && member.role.toLowerCase() !== "owner");
      ...
      <li className="p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-4 w-full sm:w-auto justify-end">
          {canManage && !isSelf ? (
            ...
            <span
              className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${roleClass}`}
            >
              {displayRole}
            </span>
          )}
