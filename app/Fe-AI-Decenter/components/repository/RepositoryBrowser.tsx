...
const RepositoryBrowser: React.FC<RepositoryBrowserProps> = ({
  ...
  const [allPublicRepositories, setAllPublicRepositories] = useState<
    Repository[]
  >([]);
  ...
  const [viewingRepository, setViewingRepository] = useState<Repository | null>(
    null
  );
    ...
    const getRepositories = async () => {
      ...
      try {
        if (organizationName) {
          ...
          setAllPublicRepositories(
            Array.isArray(publicRepos) ? publicRepos : []
          );
        }
      } catch (err) {
  ...
  const { myRepositories, communityRepositories } = useMemo(() => {
    ...
    return {
      myRepositories: myFiltered,
      communityRepositories: communityFiltered,
    };
  ...
  const renderContent = () => {
    ...
    const orgNameForDetail =
      organizationName || viewingRepository?.organization?.name;
    ...
    if (showCreateForm && organization) {
      // Can only create if we know the org
      return (
        ...
        <header className="flex flex-col md:flex-row justify-end items-center mb-6 gap-4">
          <div className="flex items-center gap-4 w-full md:w-auto md:max-w-md flex-1">
            ...
            <Button
              ...
              title={
                !organization
                  ? "Select an organization to create a repository"
                  : ""
              }
            >
