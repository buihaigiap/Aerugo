...
const RepositorySettings: React.FC<RepositorySettingsProps> = ({
  ...
  const handleUpdateSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!/^[a-z0-9]+(?:[._-][a-z0-9]+)*$/.test(formData.name)) {
      setUpdateError(
        "Name must be lowercase, alphanumeric, and can contain separators (-, _, .)."
      );
      return;
    }
    ...
    try {
      ...
      setUpdateError(
        "Failed to update repository. The new name might be taken."
      );
      console.error(err);
    } finally {
      setIsUpdating(false);
    ...
    <div className="space-y-12 animate-fade-in-up">
      <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-6">
        ...
        <form onSubmit={handleUpdateSubmit} className="space-y-6">
          ...
          {updateSuccess && (
            <p className="text-sm text-green-500">{updateSuccess}</p>
          )}
      ...
      <div className="bg-slate-800/50 border border-red-700/50 rounded-lg p-6">
        ...
        <p className="text-slate-400 mb-4">
          Deleting a repository is permanent and cannot be undone. This will
          delete all associated tags and images.
        </p>
        <Button
          variant="danger"
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-end items-center space-x-4 pt-2">
            ...
            <Button
              ...
              disabled={
                deleteConfirmationName !== repository.name || isDeleting
              }
            >
              Delete Repository
            </Button>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1">
        <span className="block text-md font-semibold text-slate-100">
          {label}
        </span>
